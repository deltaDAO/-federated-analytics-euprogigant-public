...
export function getPageBySlug(slug: string, subDir?: string): PageData {
  ...
  const fullPath = subDir ? join(pagesDirectory, subDir, `${realSlug}.md`) : join(pagesDirectory, `${realSlug}.md`);
...
export function getAllPages(subDir?: string): PageData[] {
  const slugs = fs.readdirSync(join(pagesDirectory, subDir || '')).filter((slug) => slug.includes('.md'));
